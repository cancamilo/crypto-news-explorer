...
from models import NewsAPIModel, NewsDataIOModel, TelegramMessage
...
class TelegramChannelsFetcher:
    ...
    async def fetch_news_from_telegram(self, loop) -> List[Dict]:
        print("calling fetch tele function")
        messages = []
        self.loop = loop
        self.client = TelegramClient(f"session_data/my_user", settings.TELEGRAM_API_ID, settings.TELEGRAM_API_HASH, loop=loop)
        await self.client.connect()
        print("tele connected")
        async with self.client:
            # Ensure you're authorized
            if not await self.client.is_user_authorized():
                raise Exception("Client not auhtorized")

            input_channel = "@socryptoland"
            print("fetching data for ", input_channel)
            async for msg in self.client.iter_messages(input_channel, 3):
